...
final int POINT_DELETING_MODE = 2;
final int POINT_MOVING_MODE   = 3;
final int FACE_DELETING_MODE  = 4; 
int mode = FACE_DELETING_MODE; 

String plyFilename = "hand-2014-151v-L.ply"; 
...
void setup() {
  ...
  handImg = loadImage("images/hand.jpg"); 
  bRecordPdf = false; 
  mode = FACE_DELETING_MODE;
...
void draw() {
  ...
  } else if (mode == POINT_DELETING_MODE) {
    background(200, 240, 240);
  } else if (mode == FACE_DELETING_MODE) {
    background (200, 240, 200);
  }


  image(handImg, 0, 0);
  int mousexVertexIndex = mouseX% vertices.size();
  Vertex mv = vertices.get(mousexVertexIndex); 
  fill(0, 0, 0, 40); 
  ...
  if (bDrawColorIndexed) {
    ...
    for (int i=0; i<nFaces; i++) {
      Face f = faces.get(i);
      float r = map(i, 0, nFaces-1, 96, 255); 
      float g = map(i, 0, nFaces-1, 64, 255); 
      float b = map(i, 0, nFaces-1, 255, 128); 
      fill (color (r, g, b)); 
      f.drawSimple();
    }
    popMatrix();
  ...
  String modeStr = "mode = "; 
  switch (mode) {
  case POINT_ADDING_MODE: 
    modeStr += "POINT_ADDING_MODE"; 
    break;
  case FACE_ADDING_MODE: 
    modeStr += "FACE_ADDING_MODE"; 
    break; 
  case POINT_DELETING_MODE: 
    modeStr += "POINT_DELETING_MODE"; 
    break;
  case  POINT_MOVING_MODE: 
    modeStr += "POINT_MOVING_MODE"; 
    break;
  case FACE_DELETING_MODE: 
    modeStr += "FACE_DELETING_MODE";
    break;
  }
  text(modeStr, tx, ty+=dy);


  text("# vertices = " + vertices.size(), tx, ty+=dy); 
  text("# faces = " + faces.size(), tx, ty+=dy); 
  text("mousexVertexIndex = " + mousexVertexIndex, tx, ty+=dy) ;
  text("highlightFaceIndex = " + highlightFaceIndex, tx, ty+=dy) ;
  ty+=dy/2; 
  text("'  ' to add current triangle", tx, ty+=dy);
  text ("d = delete triangle", tx, ty+=dy);
  text ("Q = POINT_ADDING_MODE", tx, ty+=dy);
  text ("W = FACE_ADDING_MODE", tx, ty+=dy);
  text ("E = POINT_MOVING_MODE", tx, ty+=dy);
  text ("R = POINT_DELETING_MODE", tx, ty+=dy); 
  text ("T = FACE_DELETING_MODE", tx, ty+=dy); 
  ty+=dy/2; 
  text ("2,3,4: subdivide current edge", tx, ty+=dy); 
...
void mousePressed() {
  ...
  if (mode == POINT_DELETING_MODE) {
    highlightVertexIndex = whichVertex;
    println ("Clicked on vertex: " + highlightVertexIndex);
  }
  if (mode == FACE_DELETING_MODE){
  ...
  if (mode == POINT_ADDING_MODE) {
    float vx = mouseX/upScale; 
    float vy = mouseY/upScale;
