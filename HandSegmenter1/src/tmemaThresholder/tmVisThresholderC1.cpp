...
tmVisThresholderC1::tmVisThresholderC1 (int w, int h){
	...
	norm_histo = new double[256]; /* normalized histogram */
...
Ipp32s*	tmVisThresholderC1::computeHistogram (Ipp8u *input, int controlFlags){
	if (input != NULL){
		...
		for (int i=0; i<nHistogramBuckets; i++){ 
			srcHistogram32s[i] = MIN(32767, srcHistogram32s[i]);
		}
		...
		if (controlFlags & THRESH_CONTROL_HIST_AVERAGE){
			...
			for (int i=0; i<nHistogramBuckets; i++){ srcHistCopyA16s[i] = (Ipp16s) srcHistogram32s[i]; }
			...
			for (int i=0; i<nHistogramBuckets; i++){ srcHistogram32s[i] = srcHistCopyA16s[i]; }
		}
...
//=================================================================================
void tmVisThresholderC1::renderHistogram (float x, float y, float w, float h, bool logarithmic){
	...
	if (!logarithmic){
		...
		Ipp32s maximumValue; 
		int indexOfMaximum;
		ippsMaxIndx_32s (srcHistogram32s, nHistogramBuckets, &maximumValue, &indexOfMaximum);
		
		glColor3f(1,1,1);
		glBegin(GL_LINES);
		float scale = h/(float)maximumValue;
		for (int i=0; i<nHistogramBuckets; i++){
			val = scale * srcHistogram32s[i];
			float x = ofMap(i, 0,nHistogramBuckets, 0,w);
			glVertex2f(x, 0);
			glVertex2f(x, val);
		}
		glEnd();
		...
		for (int i=0; i<nHistogramBuckets; i++){
			val = scale * log(1.0+srcHistogram32s[i]);
			float x = ofMap(i, 0,nHistogramBuckets, 0,w);
			glVertex2f(x, 0);
			glVertex2f(x, val);
		}
...
void	tmVisThresholderC1::modImageByHistogram (Ipp8u *input, Ipp8u *buffer_out, int slopeSign){
	if (input != NULL){
		buffer_src 	= input;
		...
		int hflags = THRESH_CONTROL_HIST_AVERAGE + THRESH_CONTROL_HIST_MEDIAN;
		...
		switch (slopeSign) {
			...
			case 0:
				...
				constant = ((double) maxBinVal) / ntotal;
				for (int i=0; i<=maxBinVal; i++){
					transform[i] = (unsigned char) (constant * distr[i] + 0.5);
					
					if ((i > 0) && (abs(transform[i] - transform[i-1]) > 128)){
						transform[i] =  transform[i-1]; // corrects an inexplicable bug! was getting a solarization for one val of i
					}
				}
				break;
	...
	if ((input != NULL) && (output != NULL)){
		...
		switch(method){
			...
			case THRESHOLD_METHOD_MIXTURE_MODELING:
				...
				break;
			case THRESHOLD_METHOD_RENYI_ENTROPY:
				theThreshold = getThresholdRenyiEntropy (buffer_src);
			case THRESHOLD_METHOD_HYBRID:
...
int tmVisThresholderC1::getThresholdHybrid (Ipp8u *input){
	if (input != NULL){
		...
		// int ty = getThresholdRenyiEntropy		(buffer_src);
...
Ipp8u*	tmVisThresholderC1::thresholdUsingValue (Ipp8u *input, Ipp8u *output, int thresh){
	if (input != NULL){
		b_threshingCalled = true;
		buffer_src = input;
		theThreshold = thresh;
...
//--------------------------------------------------------------
int	tmVisThresholderC1::getThresholdRenyiEntropy (Ipp8u *input){
...
//--------------------------------------------------------------
int tmVisThresholderC1::RenyiEntropy () {
	...
	Ipp32s *inputHistogram = srcHistogram32s;
	
	double total =0;
	for (ih = 0; ih < 256; ih++ ){
		total+=inputHistogram[ih];
	}
	
	for (ih = 0; ih < 256; ih++ ){
		norm_histo[ih] = inputHistogram[ih]/total;
	}
