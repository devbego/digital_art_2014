...
enum ThresholdMethod { 
	...
	THRESHOLD_METHOD_RENYI_ENTROPY,
...
class tmVisThresholderC1 {
	...
	public:
		...
		Ipp8u*		thresholdUsingValue 				(Ipp8u *input, Ipp8u *output, int thresh);
		...
		int			getThresholdRenyiEntropy			(Ipp8u *input);
		int 		getThresholdHybrid					(Ipp8u *input);
		
		Ipp32s*		computeHistogram 					(Ipp8u *input, int histFlags);
		void 		renderHistogram 					(float x, float y, float w, float h, bool logarithmic);
		
		void		modImageByHistogram					(Ipp8u *input, Ipp8u *output, int slopeSign); 
		...
		int			RenyiEntropy ();
		double*		norm_histo; /* normalized histogram */
