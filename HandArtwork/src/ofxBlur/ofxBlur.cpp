...
string generateBlurSource(int radius, float shape, bool bDoNotBlurBlueChannel) {
	...
	if (bDoNotBlurBlueChannel){
		src << "\tgl_FragColor.b = texture2DRect(source, tc).b; \n";
	}
	src << "}\n";
...
void ofxBlur::setup(int width, int height, int radius, float shape, int passes, float downsample) {
	bool bDoNotBlurBlueChannel = true;
	string blurSource = generateBlurSource(radius, shape, bDoNotBlurBlueChannel);
	
	/*
	string blurSourceShaderStr =
	"uniform sampler2DRect source; \n\
uniform vec2 direction; \n\
void main(void) { \n\
	vec2 tc = gl_TexCoord[0].st; \n\
	gl_FragColor  = 0.225454 * texture2DRect(source, tc); \n\
	gl_FragColor += 0.313517 * \n\
		(texture2DRect(source, tc - (direction * 1.38491)) + \n\
		 texture2DRect(source, tc + (direction * 1.38491))); \n\
	gl_FragColor += 0.0737561 * \n\
		(texture2DRect(source, tc - (direction * 3.25091)) + \n\
		 texture2DRect(source, tc + (direction * 3.25091))); \n\
	gl_FragColor.b = texture2DRect(source, tc).b; \n\
}";
	 */
	...
	blurShader.setupShaderFromSource(GL_FRAGMENT_SHADER, blurSource); // blurSource
	...
	if(passes > 1) {
		...
		/*
		string combineSourceShaderStr =
    	"uniform sampler2DRect s0,s1,s2,s3; \
		 uniform float brightness; \
		 const float scaleFactor = 0.5; \
		 void main(void) { \
			vec2 tc = gl_TexCoord[0].st; \
			gl_FragColor  = texture2DRect(s0, tc); tc *= scaleFactor; \
			gl_FragColor += texture2DRect(s1, tc); tc *= scaleFactor; \
			gl_FragColor += texture2DRect(s2, tc); tc *= scaleFactor; \
			gl_FragColor += texture2DRect(s3, tc); \
			gl_FragColor *= brightness / 4.; \
		}";
		 */
	
		if(ofGetLogLevel() == OF_LOG_VERBOSE) {
			cout << "ofxBlur is loading combine shader:" << endl << combineSource << endl; }
		combineShader.setupShaderFromSource(GL_FRAGMENT_SHADER, combineSource); //combineSource);
