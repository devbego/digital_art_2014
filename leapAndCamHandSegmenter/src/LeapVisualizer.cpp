...
//--------------------------------------------------------------
void LeapVisualizer::setColorDiagnostically (Finger::Type fingerType, Bone::Type boneType,
	...
	if (bProjectorSet){
		...
		if (dh > 0){
			...
			float boneRed   = 255.0 * abs(cos(angle));
			float boneGreen = 255.0 * abs(sin(angle));
			
			float boneBlue  = 0;
			switch (fingerType){
				case Finger::TYPE_THUMB:
					boneBlue = (32.0 * ID_THUMB);
					break;
				case Finger::TYPE_INDEX:
					boneBlue = (32.0 * ID_INDEX);
					break;
				case Finger::TYPE_MIDDLE:
					boneBlue = (32.0 * ID_MIDDLE);
					break;
				case Finger::TYPE_RING:
					boneBlue = (32.0 * ID_RING);
					break;
				case Finger::TYPE_PINKY:
					boneBlue = (32.0 * ID_PINKY);
					break;
			}
			
			ofSetColor(boneRed, boneGreen, boneBlue);
...
void LeapVisualizer::drawBone (const Finger & finger, Bone & bone,ofxLeapMotion & leap){
	...
	if (bDrawSimple){
		...
		if (bDrawDiagnosticColors){
			setColorDiagnostically (fingerType, boneType, bonePt0, bonePt1);
